...
var TimetableComponent = /** @class */ (function () {
    ...
    TimetableComponent.prototype.ngOnInit = function () {
        ...
        this.tt = this.timetable.getTimetable()
            ...
            _this.columnHeaders = [{ day: 'Days', colspanValue: 1 }, { day: 'Monday', colspanValue: 1 }, { day: 'Tuesday', colspanValue: 1 }, { day: 'Wednesday', colspanValue: 1 }, { day: 'Thursday', colspanValue: 1 }, { day: 'Friday', colspanValue: 1 }];
            _this.timeslots = [];
            console.log(data);
            _this.schedule = [];
            ...
            for (var i = 0; i < _this.schedule.length; i++) {
                for (var j = 1; j < _this.schedule[i].length; j++) {
                    var check = 0;
                    for (var k = 0; k < _this.schedule[i][j].length; k++) {
                        // tslint:disable-next-line:max-line-length
                        if (_this.schedule[i][j][k] !== null && _this.schedule[i][j][k].length !== 0 && _this.schedule[i][j][k].from !== lunchstart && (_this.convertedTime(_this.schedule[i][j][k].to) - _this.convertedTime(_this.schedule[i][j][k].from)) / _this.duration > 1) {
                            check = 1;
                            break;
                        }
                    }
